...
<div class="row">  
    <div class="col-lg-3" style="border-right: 1px sold black;">
        <div class="dropdown mr-1">
            <div class="accordion" id="course_topic" style="margin-top: 20px;">
                @for($i = 0; $i < count($courses->topics); $i++)
                    <div class="" style="background: #fff; border: none;">
                        <div class="" id="headingOne" style="background: #fff; border: none;">                            
                            <small class="" type="button" data-toggle="collapse" data-target="#collapse{{ $i }}" aria-expanded="true" aria-controls="collapse{{ $i }}" style="width: 100%; text-align: left; font-size: 14px; text-decoration: none;">
                        ...
                        <div id="collapse{{ $i }}" class="collapse" aria-labelledby="heading{{ $i }}" data-parent="#course_topic">
                            <div class="" style="overflow: auto;"> 
                                <ol class="u-text u-text-5" style="list-style-type: none; margin-top: 10px;">                               
                                @for($index = 0; $index < count($videos_lecture); $index++)
                                    @if($videos_lecture[$index]->topic_id == $courses->topics[$i]->id)
                                        @if($videos_lecture[$index]->played_state == TRUE)                                       
                                            <li class="course-list">{{ $index + 1 }}.  {{ $videos_lecture[$index]->lesson_name }} 
                                                <span class="fa fa-check-circle" style="margin-left: 10px; background: #fff; color: rgb(109, 130, 74);"></span></li>
                                        @else
                                            <li class="course-list">{{ $index + 1 }}.  {{ $videos_lecture[$index]->lesson_name }}</li>
                                        @endif
                                    @endif 
                                @endfor  
                                </ol>                               
    ...
    <div class="col-lg-9">
        <nav style="margin-top: 20px;">
            <div class="nav nav-tabs course-tab" style="border: none;" id="nav-tab" role="tablist">
                ...
                <div class="embed-responsive embed-responsive-4by3" id="video_to_play" style="border: 1px solid rgb(109, 130, 74);margin-top: 20px; background: #ccc; max-width: 93%; height: 410px;">
                    @for($index = 0; $index < count($videos_lecture); $index++)
                        @if($videos_lecture[$index]->played_state == FALSE)
                            ...
                            {{ $videos_lecture_description = $videos_lecture[$index]->lesson_description  }}
                            @break
                        @endif 
                    @endfor                             
                </div>         
                <nav aria-label="Page navigation example" style="max-width: 93%; margin-bottom: 40px;">                   
                    <small style="font-size: 14px;" id="description">
                        <p style="font-size: 18px;">Overview of what is covered in this video</p><hr>
                        {{ $videos_lecture_description }}
                    </small><hr><br>
                        ...
                        @for($i = 0; $i < count($discussions); $i++)
                            <div class="card" style="background: #fff; border: none;">
                                <div class="card-header" id="headingOne" style="background: #fff; border: none;">
                                    <h5 class="mb-0">
                                        <button class="btn" type="button" data-toggle="collapse" data-target="#collapse_{{ $i }}" aria-expanded="true" aria-controls="collapse{{ $i }}" style="color: #000; width: 100%; text-align: left; font-size: 16px; text-decoration: none;">
                                ...
                                <div id="collapse_{{ $i }}" class="collapse" aria-labelledby="heading{{ $i }}" data-parent="#course_topic">
                                    <div class="card-body" style="margin-left: 5%; ">
                                        ...
                                        <form method="POST" action="/courses/teach/discussion/comments" enctype="multipart/form-data" style="margin-top: 20px; width: 100%;">
                                            ...
                                            <div class="form-group">
                                                <button type="submit" class="btn btn-success" style="font-weight: bold; width: 20%; float: right; background: rgb(109, 130, 74);; border: none;"> <span class="fa fa-paper-plane" aria-hidden="true" style="color: #fff; margin-right: 10px;"></span>Send </button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
